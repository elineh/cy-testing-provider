...
// 1) Flexibility: the business logic (MovieService) is decoupled from the data access logic
// making it easier to swap or replace adapters (e.g. switch from Prisma to an API or..)
// without changing the business logic

// 2) Testability: this separation allows for isloated unit tests of each component
// meaning you can test the business logci independently from the data layer
  ...
  try {
    // Code that may throw an error
     ...
     if (error instanceof Error) {
         // Handle error as an instance of the Error class
      } else {
            // Handle the case where the error is not an instance of Error
            console.error("An unknonw error occurred:", error);
            }
        }
    }
  */
  ...
  async addMovie(
    ...
    try {
      ...
      const movie = await this.prisma.movie.create({
        data: id
          ? { ...data, id, year: data.year ?? 0 }
          : { ...data, id: undefined, year: data.year ?? 0 }
      })
