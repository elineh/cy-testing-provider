...
export const stateHandlers: StateHandlers & MessageStateHandlers = {
  'Has a movie with a specific ID': async (params: AnyJson) => {
    ...
    if (res.status !== 200) {
      // if the movie doens't exist, create it
      ...
      console.log(`Movie with ID ${id} already exists, skipping creating.`)
    }
  ...
  'An existing movie exists': async (params: AnyJson) => {
    ...
    if (res.status !== 200) {
      ...
      console.log(`Movie with name ${name} already exists, skipping creating.`)
    }
