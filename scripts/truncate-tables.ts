...
// Unlike TRUNCATE, DELETE logs each rows deletion, but it's the proper way to clear tables in SQLite.
...
// We reset this sequence to ensure that the IDs start from 1 again after deletion, simulating a "fresh" table.

export async function truncateTables(): Promise<void> {
  await prisma.$executeRaw`DELETE FROM "Movie"` // Clears the table by deleting all rows
  await prisma.$executeRaw`DELETE FROM sqlite_sequence WHERE name = 'Movie'` // Resets the auto-increment sequence if needed
